$(function () {
							...
							case 'validate':
								if (!res.data.validForm) {
									...
									if (res.data.inputEvent != 'blur') {
										...
										setTimeout(() => {
											clearTimeout();
											eventController.sendMessage({"event": res.data.lastEvent,
    ...
    let cssController = {
    	...
    	addExtraCssForEle: (option, apiVersion) => {
    		...
    		if (frame) {
    			eventController.sendMessage({"event": 'updateCss', pageS: pageS, extraStyle: inputS, "lastSubmitAt": new Date()}, "updateCss");
    		}
    	}
    };
					...
					if (!elementController.isMounted(elementBase.frameId)) {
						if (elementController.isMountedRelated(elementBase.formId, elementBase.formWrapperId)) {
							...
							frameEleInit.onload = () => {
								...
								resolve({"code": "0000", "msg": "Successful initialization", "data": {"paymentMethodList": (scriptParam.customerMethodsList.length > 0 ? scriptParam.customerMethodsList : [])}});
								window.onresize = () => {
    ...
    let confirmPayment = function(v) {
		return new Promise(function(resolve, reject) {
			...
			if (key == '' || key == null) {
				...
				eventController.sendMessage({"event": "confirmPayment", "lastSubmitAt": new Date(), "billing_detail": scriptParam.trnxDetail, "api_key": key}, e);
			}
	...
	let confirmCharge = function(v) {
		return new Promise(function(resolve, reject) {
			...
			if (key == '' || key == null) {
				...
				let e = "doConfirm";
				eventController.sendMessage({"event": "doConfirm", "lastSubmitAt": new Date(), "trnx_detail": scriptParam.trnxDetail, "api_key": key, "sign_key": v.signkey}, e);
			}
		})
	};
	let confirmPaymentMethod = function(v) {
		return new Promise(function(resolve, reject) {
			...
			if (key == '' || key == null) {
				...
				let e = 'submitwithValidate';
				eventController.sendMessage({"event": "confirmPaymentMethod", "lastSubmitAt": new Date(), "billing_detail": scriptParam.trnxDetail, "api_key": key}, e);
			}
		});
	};
	let googleMethod = () =>{
		...
		function onGooglePayLoaded() {
			...
			paymentsClient.isReadyToPay(getGoogleIsReadyToPayRequest())
				...
				.catch(function(err) {
					scriptParam.errorMessageEvent=eventController.getCustomEvent("getResultData", {"code":"0001","data": err,"msg":"Unsuccessful"});
					window.dispatchEvent(scriptParam.errorMessageEvent);
					console.error(err);
				});
		};
	...
	function processPayment(paymentData) {
		return new Promise(function(resolve, reject){
			...
			if(paymentToken != '' ){
				...
				{method: 'post', async: false, needToken: true,contentType: "application/json",type:sha256(window.btoa(paymentToken))},param);
				if (resp.code === '0'){
				...
				reject({"code": "0002", "msg": "The transaction result data of googlePay has not been obtained, please try to pay by credit card."});
			}
		});
	}
	...
	function authorize(data,sessionObj) {
		...
		let resp = httpController.createHttpRequest(scriptParam.requestUrl[scriptParam.scriptMode] + "services/v3/payment_methods",
		{method: 'post', async: false, needToken: true,contentType: "application/json",type:sha256(window.btoa(takenString))},param);
		...
		if (resp.code === '0'){
			...
			scriptParam.errorMessageEvent=eventController.getCustomEvent("getResultData", {"code":"0001","data": resp,"msg":"Unsuccessful"});
			window.dispatchEvent(scriptParam.errorMessageEvent);
			scriptParam.sessionObj.abort();
			scriptParam.sessionObj=null
		}
			...
			if (key == '' || key == null) {
				...
				if (scriptParam.suffix == 'v202203') {
					...
					resp = httpController.createHttpRequest(scriptParam.requestUrl[scriptParam.scriptMode] + scriptParam.suffix + "confirmCharge",
						{method: 'post', async: false, needToken: true,contentType: "application/json",confirmOptions:confirmOptions},confirmParam);
						if (resp.code === '0') {
							let re = resp.data
							if (re.code === '0000' && re.orderStatus === "-1"){ // threeDs
								if (re.threeDsType === '1') {
					...
					resp = httpController.createHttpRequest(scriptParam.requestUrl[scriptParam.scriptMode] + scriptParam.suffix + "confirmCharge",
						{method: 'post', async: false, needToken: true,contentType: "application/json",sessionToken:key},confirmParam);
						if (resp.code === '0') {
							let re = resp.data
							if (re.code === '0000' && re.orderStatus === "-1"){ // threeDs
								if (re.threeDsType === '1') {
	...
	let updateStyle = (v) => {
    	...
    	if (typeof module !== 'undefined' && module.exports) {
	        ...
	        (function(){ return this || (0,eval)('this'); }()).AsiabillPay = ABPayment_initScript;
	    }
	    !function(r, e) {
